...
namespace ProcessorInfo.Webapp.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
        ...
        public async Task<IActionResult> OnGetCallbackAsync(string returnUrl = null, string remoteError = null)
        {
            ...
            else
            {
                ...
                if (info.Principal.HasClaim(c => c.Type == ClaimTypes.Email))
                {
                    ...
                    {
                        var access_token_json = new WebClient().DownloadString("https://graph.facebook.com/oauth/access_token?client_id=1141386840572628&client_secret=6a2043f45dfe9cb080ec3c99690f0a31&grant_type=client_credentials");
                        var token = JsonConvert.DeserializeObject<TokenModel>(access_token_json);
                        var picurl = $"https://graph.facebook.com/{id}/picture?type=large&access_token={token.access_token}";
                        BlobClient blobClient = containerClient.GetBlobClient(info.Principal.FindFirstValue(ClaimTypes.Surname).Replace(" ", "").ToLower() + "_" + info.Principal.FindFirstValue(ClaimTypes.GivenName).Replace(" ", "").ToLower());
                        var wc = new WebClient();
                        var uploadFileStream = await wc.DownloadDataTaskAsync(picurl);
                        using (var ms = new MemoryStream(uploadFileStream))
                        {
                            await blobClient.UploadAsync(ms, true);
                        }
                        blobClient.SetAccessTier(AccessTier.Cool);
                        Input.PictureUrl = blobClient.Uri.AbsoluteUri;
                    }
                    else if (info.ProviderDisplayName == "Microsoft")
                    {
                        ...
                        BlobClient blobClient = containerClient.GetBlobClient(info.Principal.FindFirstValue(ClaimTypes.Surname).Replace(" ", "").ToLower() + "_" + info.Principal.FindFirstValue(ClaimTypes.GivenName).Replace(" ", "").ToLower());
                        var uploadFileStream = await wc.DownloadDataTaskAsync($"https://graph.microsoft.com/beta/users/{id}/photo/$value");
                        using (var ms = new MemoryStream(uploadFileStream))
                        {
                            await blobClient.UploadAsync(ms, true);
                        }
        ...
        public async Task<IActionResult> OnPostConfirmationAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (info.ProviderDisplayName == "Facebook")
                {
                    ...
                    user.PhotoUrl = Input.PictureUrl;
